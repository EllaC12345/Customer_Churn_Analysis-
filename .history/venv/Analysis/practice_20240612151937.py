...
def ensure_spark_dataframe(data):
    if isinstance(data, DataFrame):
        return data
    else:
        raise ValueError("Input data is not a Spark DataFrame.")

ensure_spark_dataframe(data)
...
def numeric_profile_data(data):
    # Counts
    count_df = data.agg(*[count(when(~isnan(c) & ~isnull(c), c)).alias(c) for c in rent_df.columns])
    na_count_df = rent_df.agg(*[sum(when(isnan(c) | isnull(c), 1).otherwise(0)).alias(c) for c in rent_df.columns])
    unique_count_df = rent_df.agg(*[count(col(c).distinct()).alias(c) for c in rent_df.columns])

    # Stats
    min_df = rent_df.select([min(col(c)).alias(c) for c in rent_df.columns])
    max_df = rent_df.select([max(col(c)).alias(c) for c in rent_df.columns])
    mean_df = rent_df.select([mean(col(c)).alias(c) for c in rent_df.columns])

    # Combine all
    result_df = count_df.union(na_count_df).union(unique_count_df).union(min_df).union(max_df).union(mean_df)
    result_df = result_df.toDF(*["Dtype"] + rent_df.columns)
