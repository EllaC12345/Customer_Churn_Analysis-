...
df = pd.DataFrame(list(zip(maxDepths, test_accs, train_accs)), columns = ["maxDepth", "test_accuracy", "train_accuracy"], index
                  )

df
px.bar(df, x="maxDepth", y=['test_accuracy','train_accuracy' ], index = [numerical_cols + categorical_cols_indexed], title="Model Evaluation")
fig = px.bar(df, y=, x= score, title="Model Evaluation"
             )
