...
def numeric_profile_data(data):
    ...
    na_count_exprs = [numeric_data.where(col(column).isNull()).count().alias("NA Count")]
    stat = numeric_data.describe().toPandas().transpose()
    return profile_df
...
def category_profile_data(data):
    ...
    count_exprs = [data.count().alias("Count")]
    
    # Count null values for each column
    na_count_exprs = [data.where(col(column).isNull()).count().alias("NA Count") for column in categorical_cols]
    
    # Count unique values for each column
    count_unique_exprs = [data.select(column).distinct().count().alias("Count Unique") for column in categorical_cols]
    
    # Calculate mode for each column
    mode_exprs = [data.groupBy(column).count().orderBy(col("count").desc()).limit(1).select(column).collect()[0][0].alias("Mode") for column in categorical_cols]
    
    # Combine all expressions
    exprs = count_exprs + na_count_exprs + count_unique_exprs + mode_exprs
    
    # Create the resulting DataFrame
    profile_df = spark.createDataFrame(exprs, ["Statistic"] + categorical_cols)
