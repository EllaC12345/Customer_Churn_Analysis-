...
def numeric_profile_data(data):
    numeric_columns = [col for col, dtype in data.dtypes if dtype in ['int', 'bigint', 'float', 'double']]
    # Counts
    count_df = data.agg(*[count(when(~isnan(c) & ~isnull(c), c)).alias(c) for c in numeric_columns])
    na_count_df = data.agg(*[sum(when(isnan(c) | isnull(c), 1).otherwise(0)).alias(c) for c in numeric_columns])
    unique_count_df = data.agg(*[count(col(c).distinct()).alias(c) for c in numeric_columns]).toDF(*["Count Unique"] + numeric_columns)
  

    # Stats
    min_df = data.select([min(col(c)).alias(c) for c in numeric_columns])
    max_df = data.select([max(col(c)).alias(c) for c in numeric_columns])
    mean_df = data.select([mean(col(c)).alias(c) for c in numeric_columns])

    # Combine all
    result_df = count_df.union(na_count_df).union(unique_count_df).union(min_df).union(max_df).union(mean_df)
    result_df = result_df.toDF(*["Dtype"] + data.numeric_columns)
