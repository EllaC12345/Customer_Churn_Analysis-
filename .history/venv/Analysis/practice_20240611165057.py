...
null_counts = [data.where(col(col_name).isNull()).count() if col_name in data.columns else 0 for col_name in numeric_cols]
# Exploratory Data Analysis

## Data Profiling

def numeric_profile_data(data):
    """PySpark Profiling Function

    Args:
        data (DataFrame): A PySpark DataFrame to profile

    Returns:
        DataFrame: A DataFrame with profiled data
    """
    numeric_cols = [col_name for col_name, col_type in data.dtypes if col_type in ['int',  'double']]
    profile_df = data.select(numeric_cols).describe().toPandas().transpose()
    profile_df = profile_df.rename(columns={'summary': 'Dtype'})
    profile_df['Dtype'] = 'numeric'
    
    
    #null_counts = [data.where(col(col_name).isNull()).count() if col_name in data.columns else 0 for col_name in numeric_cols]
    """
    unique_counts = [data.select(col_name).distinct().count() for col_name in numeric_cols]
    modes = [data.groupby(col_name).count().sort(col('count').desc()).first()[0] for col_name in numeric_cols] """

    #profile_df['NA Count'] = null_counts
    profile_df['Count Unique'] = unique_counts
    profile_df['Mode'] = modes """
