...
def numeric_profile_data(data):
    ...
    # Counts
    count_df = data.agg(*[count(when(~isnan(c) & ~isnull(c), c)).alias(c) for c in data.numeric_columns])
    na_count_df = data.agg(*[sum(when(isnan(c) | isnull(c), 1).otherwise(0)).alias(c) for c in data.numeric_columns])
    unique_count_df = data.agg(*[count(col(c).distinct()).alias(c) for c in data.numeric_columns])

    # Stats
    min_df = data.select([min(col(c)).alias(c) for c in data.numeric_columns])
    max_df = data.select([max(col(c)).alias(c) for c in data.numeric_columns])
    mean_df = data.select([mean(col(c)).alias(c) for c in data.numeric_columns])
