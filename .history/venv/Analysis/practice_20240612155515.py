...
def numeric_profile_data(data):
    ...
    """
            ...
            profile_df = pd.concat([
                    pd.Series(data_df.dtypes, name = "Dtype"),
                    # Counts
                    pd.Series(data_df.count(), name = "Count"),
                    pd.Series(data_df.isnull().sum(), name = "NA Count"),
                    pd.Series(data_df.nunique(), name = "Count Unique"),
                    # Stats
                    pd.Series(data_df.min(), name = "Min"),
                    pd.Series(data_df.max(), name = "Max"),
                    pd.Series(data_df.mean(), name = "Mean"),
                    pd.Series(data_df.median(), name = "Median"),
                    pd.Series(data_df.mode().iloc[0], name = "Mode"),
                ],
                axis=1
    )
    return profile_df
...
def numeric_profile_data(data):
    ...
    profile_df = pd.concat([
        pd.Series(data[numerical_columns].dtypes, name="Dtype"),
        pd.Series(data[numerical_columns].count(), name="Count"),
        pd.Series(data[numerical_columns].isnull().sum(), name="NA Count"),
        pd.Series(data[numerical_columns].nunique(), name="Count Unique"),
        pd.Series(data[numerical_columns].min(), name="Min"),
        pd.Series(data[numerical_columns].max(), name="Max"),
        pd.Series(data[numerical_columns].mean(), name="Mean"),
        pd.Series(data[numerical_columns].median(), name="Median"),
        pd.Series(data[numerical_columns].mode().iloc[0], name="Mode"),
    ], axis=1)
...
def numeric_profile_data(data):
    ...
    stat_df = numeric_data.describe().toPandas().transpose()
    return stat_df
...
def category_profile_data(data):
    ...
    profile_df = data.select(categorical_cols).summary("count", "min", "max").toPandas().transpose()
    return profile_df
