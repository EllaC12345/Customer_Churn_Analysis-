...
grouped_dfs.items()
for col, grouped_df in grouped_dfs.items():
    fig = px.bar(grouped_df, x=col, y="count", title=f"Customer Count Rate by {col}")
    fig.show()
...
for i in range(len(categorical_cols)):
    cat_fig.add_subplot(6, 3, i+1)
    plt.hist(data_df[categorical_cols[i]])
    plt.title(categorical_cols[i])

sns.countplot(x='Subscription Type', data=training_dataset)
plt.title("Countplot of Subscription Type")
